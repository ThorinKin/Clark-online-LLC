...
const PaymentSuccessPage = () => {
        ...
        const confirmPayment = async () => {
            try {
                const response = await requestWithFetch(`/api/orders/${details.orderId}/confirm`, {
                    payload: { transactionId: details.transactionId },
                ...
                setCreditsAwarded(response?.data?.creditsAwarded ?? response?.creditsAwarded ?? null);
    ...
    const renderStatusIcon = () => {
        if (status === 'pending') return <Loader2 className="w-24 h-24 text-primary mx-auto animate-spin" />;
        if (status === 'success') return <CheckCircle className="w-24 h-24 text-green-500 mx-auto" />;
